...
export class AccommodationUpdateComponent implements OnInit {
 ...
 updateAmenities(): void {
  ...
  getAccommodationDetails(): void {
    this.http.get(`https://localhost/api/accommodations/${this.accommodationId}`).subscribe(
      (response: any) => {
  ...
  submit(): void {
    ...
    this.http
      .put(`https://localhost/api/accommodations/${this.accommodationId}`, updatedData)
      .subscribe(
        () => {
